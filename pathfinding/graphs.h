 ...
 *         When a new vertex is added, this index becomes the number
 *         of vertices in the adjency list before it is added
 ...
 * @next: Pointer to the next vertex in the adjency linked list
 *        This pointer points to another vertex in the graph,
 *        but it doesn't stand for an edge between the two vertices
 */
 ...
 * To represent the connections between vertices, each vertex possess a singly
 * linked list of edge_t;
 * Each edge structure represents a connection between the vertex and the
 * @dest member of the edge structure;
 * The @next member of an edge points to the next edge in the linked list
 * of edges for a given vertex;
 * Edges are unidirectional;
 * It means that if we want to create a bidirectional connection between two
 * vertices A and B, we need to create two edges:
 * - One in the vertex A's edges linked list, pointing to the vertex B,
 * - and another one in the vertex B's edges linked list, pointing to the vertex A
