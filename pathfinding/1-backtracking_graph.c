 ...
 * to find a path from the current vertex to the target
 * if the target is found, the function adds the vertex to
 * the front of the queue representing the path;
 * it marks vertices as visited using the 'saw' array to avoid
 * revisiting them
...
/**
 * backtracking_graph - program that performs backtracking to find
 * a path from the start vertex to the target vertex in a graph
 * this function uses a depth-first search strategy and records
 * the path in a queue if one exists
